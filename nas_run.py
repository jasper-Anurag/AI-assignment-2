...
all_runs = [i for i in os.listdir(os.path.join(parent, 'outputs')) if os.path.isdir(os.path.join(parent, 'outputs', i)) and 'run_' in i]
run_num = len(all_runs) + 1
os.mkdir(os.path.join(parent, 'outputs', f'run_{run_num}'))
...
print(f"Fitness: {best_arch.fitness:.6f}", flush=True)
print(f"Conv Params: {best_arch.conv_params}, FC Params: {best_arch.fc_params}", flush=True)
print(f"Conv Penalty: {best_arch.conv_penalty:.6f}, FC Penalty: {best_arch.fc_penalty:.6f}", flush=True)
